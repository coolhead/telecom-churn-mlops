"""
Churn Pipeline (Kubeflow Pipelines v2, MLflow + MinIO)
- kfp >= 2.0 (tested with 2.4.x)
- Uses typed artifacts (Dataset, Model, Metrics)
- Logs to MLflow (artifacts in MinIO/S3)
- Simple “gate” step prints promote/hold decision

Compile:
  python churn_pipeline.py --compile              # -> churn_pipeline_1.0.json
  python churn_pipeline.py --compile --basename churn_pipeline_1.0

Optionally convert to YAML with Makefile target `yaml` (below).
"""
...
def _env_helper_code():
    # This returns a tiny function’s code we inline in each component to avoid
    # "NameError" when KFP serializes component bodies.
    return '''
def _setup_env(tracking, s3, key, secret, region="us-east-1"):
    ...
    # MinIO needs path-style addressing for many setups
...
def ingest_synthetic(
    ...
    # --- keep helpers inside the component (KFP serializes function body) ---
    __code = """%s"""  # filled below
    exec(__code)
    ...
    # write the helper code into this function
ingest_synthetic.__code__ = ingest_synthetic.__code__.replace(
    '__code = """%s"""  # filled below', f'__code = """{_env_helper_code()}"""'
)
...
preprocess_split.__code__ = preprocess_split.__code__.replace(
    '__code = """%s"""', f'__code = """{_env_helper_code()}"""'
)
...
def train_model(
    ...
    yhat_proba = pipe.predict_proba(X_train)[:, 1]
    auc_train = float(roc_auc_score(y_train, yhat_proba))
    ...
    with mlflow.start_run(run_name="train_model"):
        ...
        signature = infer_signature(X_train.head(32), pipe.predict_proba(X_train.head(32))[:, 1])
...
train_model.__code__ = train_model.__code__.replace(
    '__code = """%s"""', f'__code = """{_env_helper_code()}"""'
)
...
evaluate_model.__code__ = evaluate_model.__code__.replace(
    '__code = """%s"""', f'__code = """{_env_helper_code()}"""'
)
...
def _compile(basename: str):
    out = f"{basename}.json"
    compiler.Compiler().compile(pipeline_func=churn_pipeline, package_path=out)
    print(f"Compiled → {out}")


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("--compile", action="store_true", help="Compile the pipeline to JSON")
    p.add_argument("--basename", type=str, default="churn_pipeline_1.0",
                   help="Base name for output package (without extension)")
    args = p.parse_args()
    if args.compile:
        _compile(args.basename)
    else:
        _compile(args.basename)
