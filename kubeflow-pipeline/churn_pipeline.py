...
- Typed artifacts (Dataset, Model, Metrics) with bundled artifacts (kfp>=2.14).
- Synthetic data -> split -> train -> eval -> decision.
- MLflow logs to MinIO (S3-compatible).
...
@dsl.pipeline(name="churn-mlops-pipeline-1.0")
def churn_pipeline(
...
def _compile(path: str = "churn_pipeline_1.0.json"):
